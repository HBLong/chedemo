...
function getDays($days = 20) {
    $dates = array();
    $str = array(' 今天', ' 明天', ' 后天');
    for($i = 0; $i < $days; $i++) {
        $time = strtotime("+{$i} day");
        $dates[$time] = timeToStr($time, 'Y-m-j') . " 周" . getChineseWeek($time) . (isset($str[$i]) ? $str[$i] : '');
    }
    return $dates;
}
function getDays2($days = 20) {
    $dates = array();
    $str = array(' 今天', ' 明天', ' 后天');
    for($i = 0; $i < $days; $i++) {
        $time = strtotime("+{$i} day");
        $row = array(
            "timestamp" => $time,
            "ydate" => timeToStr($time, 'Y-m-d'),
            "date" => timeToStr($time, 'm-d'),
            "week" => "周" . getChineseWeek($time),
